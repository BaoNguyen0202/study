...
export class CategoryService {
    getList = async (request: CategoryEntitySearch): Promise<AxiosResponse<ResponseAPI<PaginatedList<CategoryEntity>>> | undefined | null> => {
        try {
            return await axios.post<ResponseAPI<PaginatedList<CategoryEntity>>>(CONFIG_URL.API + 'Category/get-by-request', request);
        } catch (error) {
